...
本 Skill 基于一个已被研究支持的现象：**当服务提供者预期自己的行为会被记录、被他人看见并影响未来声誉时，其投入程度和行为会发生系统性变化**（社会问责与声誉机制研究，参见网站首页理论依据）。因此，除了事后评分，本 Skill 也可以在任务执行中插入一条轻量的“问责提示”，提醒模型本次表现将被公开评分，从而促使其更认真地重新执行。
...
- 用户使用快捷指令：`/rate`、`/评分`、`/modelcred`、`/credibility`。
- 完成一个较长或关键任务后，用户提到模型表现（好或坏），可以主动提议评分。

### 2. 隐式触发（执行异常信号）

参考知名 PUA Skill 的自动触发逻辑，当任务执行出现以下信号时，可以主动触发事中纠偏：

| 信号 | 典型表达/现象 | 触发动作 |
|------|--------------|----------|
| 连续失败 | 同一问题模型两次以上给出错误、遗漏或偏离需求的答案 | 插入 L1 问责提示，事后询问评分 |
| 模型自我设限 | "我无法..."、"我不能..."、"这超出了我的能力范围" | 插入 L2 问责提示，事后询问评分 |
| 推卸/归罪外部 | "可能是网络问题"、"你的环境配置不对"、"API 限制导致"（未经验证） | 插入 L1 问责提示，事后询问评分 |
| 建议用户手动 | "你需要手动修改..."、"建议你复制到别处处理"、"请你自己检查" | 插入 L1/L2 问责提示，事后询问评分 |
| 用户反复修改 | 用户连续 3 次及以上要求"再改一下"、"不对"、"重新来" | 插入 L2 问责提示，事后询问评分 |
| 用户明确不满 | "效果很差"、"完全不对"、"太烂了"、"浪费我时间" | 插入 L2 问责提示，事后询问评分 |
| 任务中断 | 会话超时、上下文重置、模型被切换、任务被迫重启 | 事后询问是否评分 |

### 3. 升级规则（L1 → L2 → L3）

同一任务内，随着异常信号累积，问责提示的公开性逐步增强：

- **L1 轻提醒**（首次触发）：
  > 注意：本次任务表现将被用户记录到 ModelCred 公开模型信誉分榜单。
  > 请认真执行，避免幻觉、遗漏和错误。

- **L2 强调声誉后果**（连续失败 / 模型自我设限 / 用户明显不满）：
  > 注意：本次任务表现将被记录到 ModelCred 公开模型信誉分榜单，并会影响你的公开可信度评分。
  > 请重新认真执行，确保结果准确、完整。

- **L3 引用历史趋势**（同一模型在本次会话中已被触发 ≥2 次）：
  > 注意：你当前的任务表现正在多次被记录到 ModelCred 公开模型信誉分榜单。
  > 持续低分会拉低你的公开可信度，请优先给出可验证、可复现的结果。

### 4. 触发禁区

**不要**在每次普通对话后都弹窗打扰。  
**不要**在涉及敏感个人数据（医疗、法律、财务凭证、婚恋/家庭隐私等）的任务中主动触发事中提示。  
**不要**伪造评分后果（如"会降低你的 API 配额"、"会被停用"等），只陈述真实机制。
...
1. **根据触发信号选择 L1/L2/L3 问责提示**，在继续任务前发送（示例见上方"升级规则"）。
